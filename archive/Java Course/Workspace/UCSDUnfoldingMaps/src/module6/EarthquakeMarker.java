...
public abstract class EarthquakeMarker extends CommonMarker implements Comparable<EarthquakeMarker>
{
	...
	private void colorDetermine(PGraphics pg) {
		...
		float colorLevelLight = (depth / 100) * 255;
		float colorLevelInt = (depth / 500) * 255;
		float colorLevelDeep = (depth / 800) * 255;

		if (depth < THRESHOLD_INTERMEDIATE) {
			pg.fill(255 - colorLevelLight, 255 - colorLevelLight, 0);
		}
		else if (depth < THRESHOLD_DEEP) {
			pg.fill(0, 0, 255 - colorLevelInt);
		}
		else {
			pg.fill(255 - colorLevelDeep, 0, 0);
		}
	}
